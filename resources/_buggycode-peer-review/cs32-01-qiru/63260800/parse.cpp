...
shared_ptr<hospitalData> readCSVLineHospital(std::string theLine) {
    ...
    rating overall = rating(stod(getField(ss)));
    rating mortality = rating(getField(ss));
    rating safety = rating(getField(ss));
    rating readmission = rating(getField(ss));
    //TODO you need to read rating data

    return make_shared<hospitalData>(name, state, type, overall, mortality, readmission);
}
